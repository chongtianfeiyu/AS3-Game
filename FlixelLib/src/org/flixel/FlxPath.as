package org.flixel
{
	 ...
	 * 
	 *这是一个简单的寻路数据容器，算法不在这里!!!!!!!!!!!!!在<code>FlxTitleMap</code>里！！！！
	 * 以<code>FlxObject</code>为基类的对象可以跟随一组路径点移动。
	 * 当然在视图中也支持绘制debug视图效果.<code>FlxTilemap.findPath()</code>能够返回一个路劲对象,
	 * 你也可通过重写<code>add()</code>来支持你定义的路劲点对象
	 * 
	 * @author	Adam Atomic
	 * @作者	Adam Atomic
	 */
